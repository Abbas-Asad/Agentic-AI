...
@dataclass
class ProcessedResponse:    # Asan lafzon main Model response ki categorization (Model response aata hai, Ye class model se aaye response ko categorize karti hai)
                            # Is categorization ka faida ye hoga kay phir if else lga kr jesa response hai wesi decision making krwa skte hain
...
class SingleStepResult:
    ...
    """Items generated during this current step."""     # Ye dono smjh nhi aaye
...
class RunImpl:
    ...
    async def execute_tools_and_side_effects(
        ...
        *,
        ...
        if check_tool_use.is_final_output:  # agar final_output (bina output_type) aajaye to phir
            # If the output type is str, then let's just stringify it
            if not agent.output_type or agent.output_type is str:
                check_tool_use.final_output = str(check_tool_use.final_output)

            if check_tool_use.final_output is None:  # agar final_output None aaye
                logger.error(
    ...
    def process_model_response(
        ...
        *,
        ...
        for output in response.output:
            ...
            tools_used.append(output.name)    # tool use krny say phelay us ka naam add append krdo (both regular tool call and handoff tool call)
            print("\ntools_used:", tools_used)

            # Handoffs
            if output.name in handoff_map:
                items.append(HandoffCallItem(raw_item=output, agent=agent))      # --? item aur asal map list dono may append kia hai baqi anent may raising
        ...
        # print("\nprocessed_response:", processed_response) # custom variable return
        ...
        if multiple_handoffs:
            ...
            new_step_items.extend(
                [
                    ...
                    for handoff in run_handoffs[1:] # agar multiple handoff LLM nay kahe to wo pehle waly ko nikal lega
                ]
            )
    ...
    async def _check_for_final_output_from_tools(
        ...
        *,
        ...
        """Returns (i, final_output)."""   # Wesay is puray may ye hai kay final_output bnana hai ya nhi bnana
        if not tool_results:     # Ya to first time tool call hai to abhi koi result nhi aaya ya phir tool call may error aaya 
